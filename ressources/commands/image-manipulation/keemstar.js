  ...
  return({
    run: ((d) => {
      ...
      Jimp.read(url, (err, customImage) => {
        ...
        Jimp.read('./ressources/files/keem.png', (err, raw) => {
          ...
          new Jimp(raw.bitmap.width, raw.bitmap.height, 0xffffffff, (err, frame) => {
            ...
            customImage.resize(172, 113, (err, customImage) => {

              if(err) return main.commands.handleError(err, d);

              frame.composite(customImage, 196, 138, (err, frame) => {
    ...
    description: "Alex is a stupid nibba",
    args: "(URL | Attachment | @user)",
    category: "Fun",
    aliases: ["keem", "memestar"],
    cooldown: 1000 * 5
  });
