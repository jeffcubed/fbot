  ...
  return({
    run: ((d) => {
      ...
      Jimp.read(url, (err, customImage) => {
        ...
        Jimp.read('./ressources/files/nooseguy.png', (err, raw) => {
          ...
          new Jimp(raw.bitmap.width, raw.bitmap.height, 0xffffffff, (err, frame) => {
            ...
            customImage.resize(86, 115, (err, customImage) => {

              if(err) return main.commands.handleError(err, d);

              frame.composite(customImage, 61, 386, (err, frame) => {
    ...
    description: "i really want to kill myself",
